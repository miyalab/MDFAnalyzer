...
class ThreadPool:
    ...
    def __init__(self, max_thread_num: int = 2):
        ...
        self.__wait_function_infos    : List[ThreadPool.Info] = []
    ...
    def getWaitThreadNum(self) -> int:
        return len(self.__wait_function_infos)

    def submit(self, target: object, *args, **kwargs) -> ThreadPool.Result:
        result = ThreadPool.Result()
        self.__wait_function_infos.append(ThreadPool.Info(target, result, *args, **kwargs))
    ...
    def __callWaitFunction(self):
        ...
        with self.__call_wait_function_lock:
            if self.isRunning() and len(self.__threads) < self.max_thread_num and len(self.__wait_function_infos) > 0:
                thread = threading.Thread(target=self.__executeFunction, args=[self.__wait_function_infos.pop()])
